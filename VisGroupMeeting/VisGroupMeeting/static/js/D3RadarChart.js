...
JRadarChart.generateChart = function (
  id,
  tipId,
  data,
  R,
  maxNumber,
  Width,
  Height //JRadarChart.generateChart = function( id,tipId, data , maxNumber)
) {
  //this.d3  = d3js;
...
var RadarChart = {
  ...
  draw: function (id, tipId, data, options) {
    ...
    function render(data) {
      ...
      if (config.facet) {
        data.forEach(function (d, i) {
          ...
          vis.svg.selectAll('.polygon-vertices').attr('fill', JRadarColors[i]);
          vis.svg.selectAll('.legend-tiles').attr('fill', JRadarColors[i]);
        });
      } else {
    ...
    function updateConfig() {
      ...
      config.lowestPeak = Math.min(
        1000,
        d3.min(data, function (d) {
          return d3.min(d.axes, function (o) {
            return o.value;
          });
        })
      );
      ...
      config.maxValue =
        Math.abs(config.highestPeak) + Math.abs(config.lowestPeak);
    ...
    function buildVisComponents(data) {
      ...
      vis.svg = d3
        ...
        .attr(
          'transform',
          // 'translate(' + (-Math.sqrt(2) /2 * RadarL) + ',' + (-Math.sqrt(2) /2 * RadarL) + ')'
        ...
        group.axes.forEach(function (d, i) {
          d.coordinates = {
            ...
            x:
              (config.w / 2) *
              (1 -
                (parseFloat(Math.max(getRealValue(d.value), 0)) /
                  config.maxValue) *
